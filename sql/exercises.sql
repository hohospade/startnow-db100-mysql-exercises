...
amount, 
payment_date 
FROM payment WHERE payment_date BETWEEN '2005-05-27 00:00:00' AND '2005-05-27 23:59:59';
...
SELECT staff_id, first_name, last_name,last_update, address_id, picture, email, store_id, active, password username FROM staff WHERE password IS NOT NULL;

# 3h. Select all columns minus the password column from the staff table for rows that do not contain a password.
SELECT staff_id, first_name, last_name,last_update, address_id, picture, email, store_id, active, password username FROM staff WHERE password IS NULL;
...
select * from payment where date (payment_date) between'2005-05-25 00:00:00' and '2005-05-25 23:59:59';
...
select payment_id, payment_date, amount from payment where amount >5 limit 2000, 1000;
...
select title, description, special_features, length, rental_duration from film where special_features like '%behind the scenes%' and rental_duration between 5 and 7 order by length desc limit 10;
...
select film.title, film.description, film.release_year, film.language_id as language, language.name as language from film left join language on film.language_id=language.name;
...
select first_name, last_name, address.address, address.address2, city.city, address.district, address.postal_code from staff left join address on staff.address_id=address.address_id left join city on address.city_id=city.city_id;
